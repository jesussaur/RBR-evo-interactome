...
smo_nuclear_enrichment <- enrichGO(gene = sm_nuclear$TAIR,
                                   universe      = at_universe,
                                   OrgDb         = org.At.tair.db,
                                   keyType       = "TAIR",
                                   ont           = "BP",
                                   pAdjustMethod = "fdr",
                                   ...
                                   qvalueCutoff  = 0.05,
                                   maxGSSize = 500)
dotplot(smo_nuclear_enrichment,
        showCategory = 25)

lycophyte_tibble <- as_tibble(smo_nuclear_enrichment) %>%
...
chlorophytes_complete <- dplyr::full_join(volvox_tibble,
                                          ...
                                          by = "Description") %>%
  dplyr::select(Description, Vca, Cre) %>%
  dplyr::slice(1:49)
...
viridiplantae <- dplyr::inner_join(angiosperms_complete,
  ...
  dplyr::inner_join(ferns_complete,
                    by = "Description") %>%
  dplyr::inner_join(lycophyte_tibble,
                    by = "Description") %>%
  ...
  dplyr::inner_join(chlorophytes_complete,
                    by = "Description") %>%
  dplyr::select(Description, Ath, Bst, Sly, Zma, Osa, Svi, Atr,
                Pab, Pta, Gmo, Afi, Scu, Cri, Smo, Ppa, Sfa, Mpo, Aag,
...
viridiplantae_full <- dplyr::full_join(angiosperms_complete,
  ...
  dplyr::full_join(ferns_complete,
                   by = "Description") %>%
  dplyr::full_join(lycophyte_tibble,
                   by = "Description") %>%
